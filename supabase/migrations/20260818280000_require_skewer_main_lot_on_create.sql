...
-- ETAPA I1: remover custo manual da carne e exigir lote em NOVAS produções.
--
-- Migration incremental — 20260818270000 já foi executada e não é
-- editada. Assinatura de save_skewer_production_batch NÃO muda (mesmos 11
-- parâmetros, mesma ordem, mesmos tipos) — só CREATE OR REPLACE, sem
-- DROP FUNCTION (nenhum overload novo é criado).
--
-- Mudança no corpo, exatamente 1: nova validação logo no início —
--   if p_batch_id is null and p_lot_id is null then
--     raise exception 'Selecione um lote da carne antes de registrar a produção.';
--   end if;
-- Bloqueia CRIAR uma produção nova (p_batch_id IS NULL) sem lote da carne
-- (p_lot_id IS NULL). Deliberadamente restrita a p_batch_id IS NULL — uma
-- produção histórica que já existe sem lote (criada antes desta regra)
-- continua podendo ser aberta/editada/salva normalmente sem lote (o ramo
-- "p_lot_id is null" mais abaixo, que valida p_total_cost manual, permanece
-- intocado e continua sendo alcançado normalmente ao editar esse tipo de
-- produção). NUNCA usar "if p_lot_id is null" sozinho aqui — isso quebraria
-- toda produção histórica sem lote.
--
-- Tudo o mais é idêntico à versão já executada (20260818270000): validação
-- de produto/ingrediente, resolução de componentes (Fases 1/3, ingredient/
-- recipe/supply — inalteradas), lock unificado (Fase 2), resolução do lote
-- da carne principal por purchase_item.product_id (Fase 3.5, ajuste
-- anterior, não tocada nesta migration), consumo = gross_weight_g inteiro
-- (nunca usable_weight_g), custo = gross_weight_g × lots.unit_cost_base
-- (nunca confiado ao client quando há lote), reconciliação old_net/new_net
-- (Fase 4), persistência de componentes (Fase 5).
--
-- NÃO altera schema (skewer_production_batches, skewer_batch_components,
-- lots, lot_movements) — total_cost continua coluna normal, snapshot
-- persistido, nunca GENERATED. NÃO altera save_side_production_batch,
-- delete_skewer_production_batch, delete_side_production_batch,
-- apply_skewer_production_cost, apply_side_production_cost. NÃO altera
-- Compras (save_purchase, finalize_purchase_item,
-- create_lot_from_purchase_line, purchase_items, production_supplies) nem
-- Pedidos (create_customer_order, products.stock_quantity,
-- stock_movements) nem product_costs. NÃO altera o CHECK de
-- skewer_batch_components (supply continua sem lot_id — fora de escopo,
-- fica para uma etapa futura). p_total_cost permanece na assinatura só por
-- compatibilidade — auditado: removê-lo exigiria trocar a assinatura
-- inteira (novo overload/DROP FUNCTION) sem nenhum ganho real, já que ele
-- já é ignorado sempre que p_lot_id não é nulo (comportamento herdado da
-- H2, inalterado aqui).
...
begin
  ...
  -- qualquer produção histórica que já nasceu sem lote (item 6/12/21 do
  -- pedido). Editar um batch já existente sem lote continua permitido e
  -- cai normalmente no ramo "p_lot_id is null" (validação de
  -- p_total_cost manual) logo abaixo, sem nenhuma mudança de
  -- comportamento.
  if p_batch_id is null and p_lot_id is null then
    raise exception 'Selecione um lote da carne antes de registrar a produção.';
  end if;
  ...
  -- ================================================================
  -- Fase 1 — idêntica à G2/H2 já executadas: parse + validação
  -- estrutural de cada componente. recipe/supply/ingredient sem lote
  -- resolvidos direto; ingredient COM lote só grava intenção em
  -- tmp_skewer_pending_lots (nada mutável do lote é lido aqui, só
  -- depois do lock). Este fluxo (componentes) usa purchase_items.
  -- ingredient_id e NÃO é afetado por esta migration.
  -- ================================================================
  ...
  -- ================================================================
  -- Fase 3 — resolve componentes pendentes pós-lock (idêntico à G2/H2 —
  -- usa purchase_items.ingredient_id, não afetado por esta migration).
  -- ================================================================
  ...
  -- vira componente). Valida purchase_item.product_id contra
  -- p_product_id (ajuste anterior, inalterado nesta migration). Consumo
  -- = p_gross_weight_g inteiro, nunca p_usable_weight_g. Custo derivado
  -- aqui, nunca confiado ao client.
  -- ================================================================
  ...
  -- ================================================================
  -- Fase 5 — substitui os componentes por completo (idêntico à G2/H2). A
...
-- CREATE OR REPLACE preserva o mesmo OID/assinatura (nenhum parâmetro
-- adicionado/removido) — os GRANTs já concedidos permanecem válidos sem
-- precisar ser reconcedidos. Mantido de forma defensiva mesmo assim,
-- mesmo padrão já usado no restante do projeto (nunca assumir estado de
-- GRANT sem reafirmar).
...
-- em particular não precisa de nenhuma mudança, continua revertendo o
-- consumo da carne pelo ledger normalmente (source_type='skewer_production',
-- lot_id), independente de como a criação foi validada. skewer_batch_
-- components, lots, lot_movements (schema): inalterados. Compras
-- (save_purchase, finalize_purchase_item, create_lot_from_purchase_line,
-- purchase_items, production_supplies), Pedidos (create_customer_order,
-- products.stock_quantity, stock_movements), product_costs: não tocados.
--
-- total_cost continua coluna normal (numeric(12,4)), snapshot persistido,
-- nunca GENERATED — nenhuma mudança de schema. skewer_production_batches.
-- ingredient_id permanece na tabela, nullable, vestigial.
