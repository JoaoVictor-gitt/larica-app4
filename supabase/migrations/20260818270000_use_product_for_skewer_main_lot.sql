...
-- AJUSTE DE ARQUITETURA (após H2/H3): carne principal passa a usar o
-- vínculo PRODUTO (purchase_items.product_id), não mais ingredient_id.
--
-- Motivo: no cadastro real de Compras, as carnes principais usadas em
-- Produção de Espetos são cadastradas como purchase_item com vínculo =
-- Produto (product_id), não vínculo = Ingrediente. A validação da H2
-- (lot -> purchase_item -> purchase_item.ingredient_id = p_ingredient_id)
-- nunca batia com esse cadastro real — o campo de ingrediente da carne
-- era, na prática, sempre desnecessário/incorreto para este fluxo.
--
-- Migration incremental — 20260818260000 (H2) já foi executada e não é
...
--
-- Mudanças no corpo, exatamente 2:
--   1. Remove a validação "p_lot_id is not null and p_ingredient_id is
--      null" — para a carne principal, p_ingredient_id deixa de ser
--      exigido quando há lote (o próprio p_product_id, já obrigatório e
--      validado, é a referência técnica suficiente).
--   2. Fase 3.5 (resolução/validação do lote da carne principal, pós-lock)
--      passa a comparar purchase_item.product_id com p_product_id, em vez
--      de purchase_item.ingredient_id com p_ingredient_id. Nova mensagem:
--      'O lote selecionado não pertence ao produto da carne principal.'
--
-- Tudo o mais é idêntico à versão já executada: Fases 1/2/3/4/5 dos
-- componentes (ingredient/recipe/supply, incluindo o seletor de lote por
-- purchase_items.ingredient_id — ESSE fluxo continua exatamente igual,
-- não é afetado por esta migration), reconciliação old_net/new_net,
-- consumo = gross_weight_g inteiro (nunca usable_weight_g), custo =
-- gross_weight_g × lots.unit_cost_base, disciplina de lock (batch primeiro,
-- depois todos os lots envolvidos em uma única passada ORDER BY id FOR
-- UPDATE, nunca lendo dado mutável do lote antes do lock), e a checagem
-- de reversão nunca ultrapassar initial_quantity do lote (já presente na
-- versão executada da H2).
--
-- NÃO altera schema (skewer_production_batches, skewer_batch_components,
-- lots, lot_movements). NÃO altera save_side_production_batch,
...
-- delas depende de como p_lot_id da carne foi validado no save. NÃO
-- altera Compras (save_purchase, finalize_purchase_item,
-- create_lot_from_purchase_line), Pedidos (create_customer_order,
-- products.stock_quantity, stock_movements) nem product_costs. NÃO
-- altera a coluna skewer_production_batches.ingredient_id — continua
-- existindo, nullable, vestigial (não removida nesta etapa).
...
begin
  ...
  -- vira componente). AJUSTE: valida purchase_item.product_id contra
  -- p_product_id (não mais ingredient_id/p_ingredient_id) — reflete o
  -- cadastro real de Compras, onde a carne principal é vinculada por
  -- Produto. Consumo = p_gross_weight_g inteiro, nunca p_usable_weight_g.
  -- Custo derivado aqui, nunca confiado ao client.
  -- ================================================================
...
-- adicionado/removido) — os GRANTs já concedidos em 20260818260000
-- permanecem válidos sem precisar ser reconcedidos. Mantido de forma
-- defensiva mesmo assim, mesmo padrão já usado no restante do projeto
-- (nunca assumir estado de GRANT sem reafirmar).
...
-- apply_side_production_cost: inalteradas. skewer_batch_components,
-- lots, lot_movements (schema): inalterados. Compras (save_purchase,
...
-- product_costs: não tocados.
--
-- skewer_production_batches.ingredient_id permanece na tabela, nullable,
-- vestigial — não removida nesta etapa. Client (js/producao.js) ajustado
-- separadamente para escopar o seletor "Lote da carne" pelo Produto
-- selecionado (purchase_items.product_id), nunca mais por ingredient_id.
