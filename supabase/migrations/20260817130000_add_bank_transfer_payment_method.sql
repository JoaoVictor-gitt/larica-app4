-- Adiciona Transferência Bancária (bank_transfer) como forma de pagamento,
-- seguindo o mesmo padrão já usado pelo Revolut (toggle + dados em
-- business_settings, confirmação manual pela equipe, payment_status
-- 'pending'). Não altera nenhuma regra existente de horário, delivery,
-- produtos/is_available, estoque, combos, cupom, preço/troco ou o
-- comportamento de cash/card/revolut.

-- =============================================================
-- 1. Novas colunas em business_settings (aditivo, default seguro)
-- =============================================================

ALTER TABLE public.business_settings
  ADD COLUMN bank_transfer_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN bank_transfer_beneficiary text,
  ADD COLUMN bank_transfer_iban text,
  ADD COLUMN bank_transfer_bic text;

-- =============================================================
-- 2. GRANTs pras 4 colunas novas — CONFIRMADO no Supabase real:
-- business_settings usa privilégio por coluna; anon só tem SELECT nas
-- colunas públicas, authenticated tem SELECT/INSERT/UPDATE por coluna
-- (mesmo padrão já usado por revolut_enabled/revolut_qr_path/
-- revolut_payment_code). anon nunca recebe INSERT/UPDATE, e nunca é
-- concedido SELECT da tabela inteira.
-- =============================================================

GRANT SELECT (bank_transfer_enabled, bank_transfer_beneficiary, bank_transfer_iban, bank_transfer_bic)
  ON public.business_settings TO anon, authenticated;

GRANT INSERT (bank_transfer_enabled, bank_transfer_beneficiary, bank_transfer_iban, bank_transfer_bic)
  ON public.business_settings TO authenticated;

GRANT UPDATE (bank_transfer_enabled, bank_transfer_beneficiary, bank_transfer_iban, bank_transfer_bic)
  ON public.business_settings TO authenticated;

-- =============================================================
-- 3. Constraint de orders.payment_method — descobre e substitui a
-- constraint CHECK existente (qualquer que seja o nome atual), sem
-- deletar nada além dela. Só funciona se payment_method for text/varchar
-- com CHECK; se for um ENUM nativo, esta migration não altera nada
-- (v_constraint_name fica null) e precisa de ALTER TYPE em separado.
-- =============================================================
...
BEGIN
  ...
  ALTER TABLE public.orders
    ...
    CHECK (payment_method IN ('card', 'cash', 'revolut', 'bank_transfer'));
END $$;

-- =============================================================
-- 4. create_customer_order — mesmo corpo da migration anterior
-- (20260817120000_enforce_product_availability_in_orders.sql), com
-- exatamente 3 alterações: leitura de business_settings ganha as 4
-- colunas novas; validação de payment_method aceita 'bank_transfer' e
-- passa a checar bank_transfer_enabled + os 3 campos obrigatórios;
-- payment_status = 'pending' também para bank_transfer. Todo o resto
-- (disponibilidade do negócio, horário, delivery/collection, delivery
-- quote, produtos/active/is_available, locking FOR UPDATE, combos,
-- estoque, stock_movements, cupom, subtotal/total/troco, snapshot,
-- retorno JSON) é idêntico, char a char, ao arquivo já aplicado.
-- =============================================================
