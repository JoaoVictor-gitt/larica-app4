-- Integração de Lotes de Compra com Produção — ETAPA I3: insumos
-- (item_type='supply') com lote/estoque na Produção de Espetos.
--
-- Generaliza o que já existe para ingredient (Etapas G2/G4) e para a carne
-- principal do header (H2/I1) para também cobrir componentes
-- item_type='supply' (ex. palito) — reaproveitando a MESMA arquitetura já
-- validada: tabela temporária de "pendentes" resolvida só depois do lock
-- unificado de lotes, reconciliação old_net/new_net por lot_id, mesmas
-- mensagens de erro genéricas já usadas para ingredient.
--
-- NÃO altera side_batch_components/save_side_production_batch (Acompanha-
-- mentos não tem item_type='supply'). NÃO altera
-- delete_skewer_production_batch/delete_side_production_batch (G3, já
-- confirmada agnóstica a item_type — reverte por lot_id agregando do
-- ledger, sem nenhuma mudança necessária). NÃO altera
-- apply_skewer_production_cost/apply_side_production_cost, save_purchase,
-- finalize_purchase_item, create_lot_from_purchase_line, purchase_items,
-- production_supplies, lots, lot_movements (schema). Migrations já
-- executadas não são editadas.

-- =============================================================
-- 1. CHECK relaxado — mesma constraint, nome reaproveitado. Só
-- 'ingredient'/'supply' podem ter lot_id; 'recipe' nunca (decisão já
-- fixada desde G1 — ficha técnica não representa matéria-prima comprada).
-- =============================================================

ALTER TABLE public.skewer_batch_components
  DROP CONSTRAINT skewer_components_lot_id_somente_ingredient;

ALTER TABLE public.skewer_batch_components
  ADD CONSTRAINT skewer_components_lot_id_somente_ingredient
  CHECK (
    lot_id IS NULL
    OR item_type IN ('ingredient', 'supply')
  );

-- =============================================================
-- 2. save_skewer_production_batch — mesma assinatura de 11 parâmetros
-- (CREATE OR REPLACE, sem DROP FUNCTION, sem overload novo). Generaliza a
-- Fase 1 (parser)/Fase 3 (resolução pós-lock) para tratar item_type=
-- 'supply' com lot_id da mesma forma que 'ingredient' — resolvendo
-- purchase_items.production_supply_id (não .ingredient_id), unit_cost de
-- lots.unit_cost_base (nunca production_supplies.cost_per_unit), unit
-- exatamente igual a lots.base_unit (nunca hardcoda 'un'). Fases 2/4/5
-- já eram agnósticas a item_type — nenhuma mudança nelas.
-- =============================================================
...
-- adicionado/removido) — os GRANTs já concedidos permanecem válidos.
-- Reafirmados de forma defensiva mesmo assim, mesmo padrão já usado em
-- toda alteração desta RPC nesta sessão (H2/I1).
...
-- =============================================================
--
-- side_batch_components/save_side_production_batch,
-- delete_skewer_production_batch/delete_side_production_batch (G3, já
-- confirmada agnóstica a item_type — reverte por lot_id agregando do
-- ledger, nenhuma mudança necessária), apply_skewer_production_cost/
-- apply_side_production_cost, save_purchase, finalize_purchase_item,
-- create_lot_from_purchase_line: inalterados. purchase_items,
-- production_supplies, lots, lot_movements (schema): inalterados.
-- Compras (compras.html/js/css, purchases-service.js): não tocados.
